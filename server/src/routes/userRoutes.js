...
router.get(
    ...
    userController.getMyFavoritePosts
);
...
router.get("/likes", authMiddleware.verifyToken, async (req, res) => {
    try {
        ...
        res.status(500).json({
            message: "Erro interno do servidor ao buscar likes do usuário.",
        });
    }
});

router.get("/favorites", authMiddleware.verifyToken, async (req, res) => {
    try {
        const [rows] = await pool.query(
        ...
        res.status(500).json({
            message: "Erro interno do servidor ao buscar favoritos do usuário.",
        });
    }
});
